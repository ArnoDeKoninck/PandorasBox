    ...
    <Card style={{backgroundColor: grey[800]}}> 
        <CardContent>
            <Grid container rowSpacing={2} className={classes.headerTitle}>
                <Grid container >
                    <Grid item>
                        <Typography component={"span"}>{amount} {item.spellScroll? `Scroll of ${name}`:name}</Typography>
                        {description && <Typography component={"p"}>{description}</Typography>}
                    </Grid>
                    <Grid item xs={12}>
                        {link &&<Link href={link} color={customTheme.palette.secondary.main}>Read more</Link>}
                    </Grid>
